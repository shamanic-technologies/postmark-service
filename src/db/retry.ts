 ...
 *  - pg client/pool connectionTimeoutMillis expiry messages
 *
...
export function isTransientConnectError(err: unknown): boolean {
  ...
  return typeof message === "string" && /timeout expired|timeout exceeded when trying to connect/i.test(message);
}
