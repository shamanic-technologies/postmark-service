...
describe("POST /internal/transfer-brand", () => {
  ...
  it("should return 401 without API key", async () => {
    const res = await request(app)
      ...
      .send({ brandId: "b", sourceOrgId: "s", targetOrgId: "t" });
  ...
  it("should execute update and return row count", async () => {
    mockExecute.mockResolvedValueOnce({ rowCount: 5 });

    const res = await request(app)
      .post("/internal/transfer-brand")
      .set(headers)
      .send({ brandId: "brand-uuid", sourceOrgId: "src-org", targetOrgId: "tgt-org" });
