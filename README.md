...
### Webhooks
- **POST /webhooks/postmark** - Public Postmark webhook endpoint. Persists Delivery, Bounce, Open, Click, SpamComplaint, SubscriptionChange events to local storage. `RecordType=Inbound` is forwarded byte-for-byte to `${EMAIL_GATEWAY_SERVICE_URL}/inbound/postmark` with header `x-api-key: ${EMAIL_GATEWAY_SERVICE_API_KEY}`. On non-2xx or network error from email-gateway, the route returns **502** — Postmark's own 45-min retry handles redelivery. No outbox, no queue. Idempotency is the downstream consumer's responsibility.
